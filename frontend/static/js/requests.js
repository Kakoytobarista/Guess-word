...
export const createLink = async function() {
    ...
    if (await is_not_more_then_13_length()){
        ...
        const response = await fetch('http://51.250.90.3/api/word/', {
        method: 'POST',
        headers: {
...
export const getRandomWord = async function() {
  try {
    let response = await fetch('http://51.250.90.3/api/word/random_word/', {
          method: 'GET',
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
...
export const getConcreteWord = async function(uuid) {
  try {
    const response = await fetch('http://51.250.90.3/api/word?' + uuid, {
          method: 'GET',
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
